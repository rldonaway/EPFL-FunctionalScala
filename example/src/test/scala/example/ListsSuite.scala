 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of 1 number") {
    ...
    assert(sum(List(1)) === 1)
    assert(sum(List(-1)) === -1)
    assert(sum(List(217)) === 217)
  }

  test("sum including negative numbers") {
    assert(sum(List(1,-1)) === 0)
    assert(sum(List(3,-2,-1)) === 0)
    assert(sum(List(-3,2,1)) === 0)
    assert(sum(List(-1,0,0)) === -1)
    assert(sum(List(-3,-1,-2)) === -6)
  }
  ...
  test("max of no numbers") {
    intercept[NoSuchElementException](max(null))
    intercept[NoSuchElementException](max(List()))
  }

  test("max of 1 number") {
    assert(max(List(0)) === 0)
    assert(max(List(1)) === 1)
    assert(max(List(-1)) === -1)
    assert(max(List(217)) === 217)
  }

  test("max including negative numbers") {
    assert(max(List(1,-1)) === 1)
    assert(max(List(3,-2,-1)) === 3)
    assert(max(List(-3,2,1)) === 2)
    assert(max(List(-1,0,0)) === 0)
    assert(max(List(-3,-1,-2)) === -1)
  }
