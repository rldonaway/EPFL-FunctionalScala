...
object Huffman {
  ...
  def timesAccAcc(char: Char, list: List[(Char, Int)]): List[(Char, Int)] = {
    if (list.isEmpty) {
      ...
      list.head :: timesAccAcc(char, list.tail)
    }
  }
  ...
  def findMinFreqAcc(minSoFar: (Char, Int), list: List[(Char, Int)]): (Char, Int) = {
   ...
   */
  def until(done: (List[CodeTree]) => Boolean, doThis: (List[CodeTree]) => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.isEmpty || done(trees)) trees
    else until(done, doThis)(doThis(trees))
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    def frequencies: List[(Char, Int)] = times(chars)
    def orderedLeafList: List[Leaf] = makeOrderedLeafList(frequencies)
    def codeTreeList: List[CodeTree] = until(singleton, combine)(orderedLeafList)
    codeTreeList.head
  }
